...
// ── View-model types (Task 7) ───────────────────────────────────────────────
// BuilderWorkspaceLive is the WIRED copy of BuilderWorkspace. The wireframe
// infers its view-models from module-level seed arrays; here we declare them
// explicitly and accept real data as props. The `page.tsx` server component maps
// DB rows → these shapes.

// The lesson-page difficulty is stored in the DB as 'facil'|'normal'|'dificil'
// but the UI toggle uses the accented Spanish labels. Mapping happens at the
// page (DB→VM) and on save (VM→DB) boundaries.
export type BuilderDifficulty = 'fácil' | 'normal' | 'difícil';

export type BuilderLessonRef = {
  section: string;
  quote: string;
  color: string;
  sentence?: string;
...
export type BuilderAnswer = { id?: number; text: string; correct: boolean };
...
export type BuilderFile = { id?: number; name: string; size: string; status: string };
...
// VM difficulty (accented) → DB literal.
const DIFFICULTY_TO_DB: Record<BuilderDifficulty, 'facil' | 'normal' | 'dificil'> = {
  'fácil': 'facil',
  normal: 'normal',
  'difícil': 'dificil',
};
...
export function BuilderWorkspaceLive({
  ...
  // A ?lessonId= param (set when a lesson is created, mirrors ?questionId=) picks
  // which lesson is active so a reload lands back on it. Falls back to the first.
  ...
  const [questionLessonId, setQuestionLessonId] = useState(() => initialLessons[0]?.id ?? -1);
  // When navigating here from the quote sidebar, a ?questionId= param tells us
  // which question to pre-select. Fall back to the first question in scope.
  const [activeQuestionId, setActiveQuestionId] = useState(() => {
    const firstInScope = initialQuestions.find(
      (q) => q.lessonId === (initialLessons[0]?.id ?? -1),
    );
    const fallback = firstInScope?.id ?? initialQuestions[0]?.id ?? -1;
    const param = searchParams.get('questionId');
    if (!param) return fallback;
    const parsed = parseInt(param, 10);
    const found = initialQuestions.find((q) => q.id === parsed);
    return found ? parsed : fallback;
  });
  ...
  // ── Wired persistence (Task 7) ─────────────────────────────────────────────
  // Creates a new lesson in the DB immediately (with sensible defaults), then
  // selects it. Adding a ?lessonId= param means a reload lands back on the new
  // lesson so its title is visible in the header/breadcrumb.
  const createNewLesson = async () => {
    const nextOrder =
      lessonsList.reduce((max, l) => Math.max(max, l.order), 0) + 1;
    try {
      ...
      const vm: BuilderLesson = {
        id: created.id,
        title: created.title,
        subtitle: created.subtitle ?? "",
        hero: (created.hero as BuilderHero) ?? { ...DEFAULT_HERO },
        order: created.order,
        duration: created.estimatedDurationMinutes ?? 5,
        xp: created.xpReward,
        type: created.type,
        content: created.contentText ?? "",
        files: [],
      };
      setLessonsList((prev) => [...prev, vm]);
      setActiveLessonId(created.id);
      setSaveStatus("idle");
      // Reflect the new lesson in the URL WITHOUT a router navigation/refresh:
      // router.refresh() re-fetches the RSC and remounts this client component,
      // which would wipe the teacher's in-progress edits. history.replaceState
      // only updates the address bar, so a manual reload still lands here.
      if (typeof window !== "undefined") {
        window.history.replaceState(
          null,
          "",
          `/teach/${subjectId}/${unitId}?tab=lessons&lessonId=${created.id}`,
        );
      }
    } catch (error) {
      toast({
  ...
  const saveLesson = async () => {
    ...
    try {
      await updateLesson(activeLesson.id, {
        ...
        hero: activeLesson.hero,
        order: activeLesson.order,
        type: activeLesson.type,
        contentText: activeLesson.content,
        estimatedDurationMinutes: activeLesson.duration,
      ...
      toast({
        variant: "destructive",
        title: "No se pudo guardar la lección",
        description: (error as Error).message,
      });
    }
  };
  ...
  const saveQuestion = async () => {
    if (questionSaveStatus !== "dirty" || !activeQuestion) return;
    ...
    const payload = {
      ...
      difficulty: DIFFICULTY_TO_DB[q.difficulty] ?? "normal",
      answers: q.answers.map((a) => ({ name: a.text, correct: a.correct })),
    };
    try {
      if (q.id < 0) {
        // Unsaved (temp negative id) → insert.
        const newId = await addQuestionWithAnswers({
          ...payload,
          unitId,
          lessonId: q.lessonId ?? null,
          lessonRef: q.lessonRef ?? null,
        });
        setQuestionsList((prev) =>
          prev.map((x) => (x.id === q.id ? { ...x, id: newId, dirty: false } : x)),
        );
        ...
        setQuestionsList((prev) =>
          prev.map((x) => (x.id === q.id ? { ...x, dirty: false } : x)),
        );
      }
  ...
  const createNewQuestion = () => {
    // Temp NEGATIVE id marks an unsaved question; saveQuestion() inserts it and
    // swaps in the real DB id. Guards against colliding with real serial ids.
    const tempId = Math.min(0, ...questionsList.map((q) => q.id)) - 1;
    const newQ: BuilderQuestion = {
      ...
      text: "¿Enunciado de la nueva pregunta?",
      difficulty: "normal",
      // New questions belong to the lesson currently being authored (or none if
      // the unit has no lessons yet).
      lessonId: questionLessonId > 0 ? questionLessonId : null,
      lessonRef: null,
      explanation: "",
      answers: [
        { text: "Opción A", correct: true },
  ...
  const deleteQuestion = async (id: number) => {
    ...
    // Unsaved (temp) questions only exist locally.
    if (id < 0) {
      applyLocal();
  ...
  const removeFile = async (fileName: string) => {
    const file = activeLesson.files.find((f) => f.name === fileName);
    handleLessonChange({
      files: activeLesson.files.filter((f) => f.name !== fileName),
    });
    // Persisted resources (with a real id) are removed via the resources API;
    // mock/unsaved ones only exist locally (media upload is Phase 2).
    if (file?.id && activeLesson.id > 0) {
      try {
        await fetch(`/api/lessons/${activeLesson.id}/resources`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ resourceId: file.id }),
        });
      } catch {
        /* best-effort; keep the optimistic local removal */
      }
    }
  };
        ...
        <TabsContent value="questions" className="mt-4 outline-none">
          <div className="space-y-4">
            ...
            <section className="space-y-5 rounded-card border bg-card p-5 shadow-card md:p-6">
              ...
              {/* Etiqueta — short internal name for the question, surfaced in the
                  navigator and syllabus. Persisted to questions.label. */}
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Ej. Edad de plena capacidad"
